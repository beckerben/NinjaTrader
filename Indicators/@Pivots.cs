...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	public class Pivots : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description				= Custom.Resource.NinjaScriptIndicatorDescriptionPivots;
				Name					= Custom.Resource.NinjaScriptIndicatorNamePivots;
			...
			else if (State == State.Historical)
			{
				...
				if ((BarsPeriod.BarsPeriodType == BarsPeriodType.Day || (BarsPeriod.BarsPeriodType is BarsPeriodType.HeikenAshi or BarsPeriodType.PriceOnVolume or BarsPeriodType.Volumetric && BarsPeriod.BaseBarsPeriodType == BarsPeriodType.Day)) && pivotRangeType == PivotRange.Daily)
				{
		...
		protected override void OnBarUpdate()
		{
			...
			if ((priorDayHlc == HLCCalculationMode.DailyBars && BarsArray[1].DayCount <= 0)
				...
				|| ((BarsPeriod.BarsPeriodType == BarsPeriodType.Day || (BarsPeriod.BarsPeriodType is BarsPeriodType.HeikenAshi or BarsPeriodType.PriceOnVolume or BarsPeriodType.Volumetric && BarsPeriod.BaseBarsPeriodType == BarsPeriodType.Day)) && pivotRangeType == PivotRange.Daily)
				|| ((BarsPeriod.BarsPeriodType == BarsPeriodType.Day || (BarsPeriod.BarsPeriodType is BarsPeriodType.HeikenAshi or BarsPeriodType.PriceOnVolume or BarsPeriodType.Volumetric && BarsPeriod.BaseBarsPeriodType == BarsPeriodType.Day)) && BarsPeriod.Value > 1)
		...
		private DateTime RoundUpTimeToPeriodTime(DateTime time, PivotRange pivotRange) =>
			pivotRange == PivotRange.Weekly ? Gui.Tools.Extensions.GetEndOfWeekTime(time) : pivotRange == PivotRange.Monthly ? Gui.Tools.Extensions.GetEndOfMonthTime(time) : time;
			...
			if (propertyDescriptorCollection != null)
				foreach (PropertyDescriptor thisDescriptor in propertyDescriptorCollection)
					adjusted.Add(thisDescriptor.Name is "UserDefinedClose" or "UserDefinedHigh" or "UserDefinedLow"
						? new PropertyDescriptorExtended(thisDescriptor, _ => value, null, new Attribute[] { new BrowsableAttribute(false) }) : thisDescriptor);
			return adjusted;
		}
	}
}
